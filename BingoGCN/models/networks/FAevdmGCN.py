...
class FAdmGCN(nn.Module):
    def __init__(self, args):
        super(FAdmGCN, self).__init__()
    ...
    def forward(self, x, adj, sparsity=None,epoch=None):
        ...
        if (self.args.evatime == True):
            print('For torch matmul out = AX: adj size is', adj.size())
            adj_sparsity = torch.sum(adj == 0).item() / adj.numel()
            print('adj sparsity is', adj_sparsity)
            Xsparsity = torch.sum(x == 0).item() / x.numel()
            print('For torch matmul out = AX: X size is', x.size())
            print("(X Sparsity is:", Xsparsity)
            # ---
            n_mul_tot = sparse_mul_counter(adj, x)
            print("AX's non-zero mul+add operations is:", n_mul_tot)
            # ---

        x = torch.sparse.mm(adj, x)  #  out = AX
        if (self.args.evatime == True):
            print("AX's result size is:", x.size())
        x = self.layers_GCN[i](x, threshold)   # out = self.lin(x): (AX) W
        # ....
        if (self.args.evatime == True):
            AXWsparsity = torch.sum(x == 0).item() / x.numel()
            print('For torch matmul out = (AX)W: out size is', x.size())
            print("(AX)W's Sparsity is:", AXWsparsity)
        ...
        if (self.args.evatime == True):
            ...
            AXsparsity = torch.sum(tmp == 0).item() / tmp.numel()
            print('For torch matmul out = AX: out size is', tmp.size())
            print("AX's Sparsity is:", AXsparsity)

        x = torch.matmul(adj, x)  #  out = AX
        x = self.layers_GCN[-1](x, threshold)  # out = (AX)W
        if (self.args.evatime == True):
            AXWsparsity = torch.sum(x == 0).item() / x.numel()
            print('For torch matmul out = (AX)W: out size is', x.size())
            print("(AX)W's Sparsity is:", AXWsparsity)

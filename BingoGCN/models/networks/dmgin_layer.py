    ...
    def __init__(self, num_layers, input_dim, hidden_dim, output_dim,args):
        ...
        if num_layers < 1:
            ...
            if (args.enable_mask == True):
                self.linear = SparseLinearMulti_mask(input_dim, output_dim, bias=False,args=args)
            else:
            ...
            if (args.enable_mask == True):
                self.linears.append(SparseLinearMulti_mask(input_dim, hidden_dim, bias=False,args=args))
            else:
                self.linears.append(SparseLinear(input_dim, hidden_dim, bias=False,args=args))
            for layer in range(num_layers - 2):
                if (args.enable_mask == True):
                    self.linears.append(SparseLinearMulti_mask(hidden_dim, hidden_dim, bias=False,args=args))
                else:
                    self.linears.append(SparseLinear(hidden_dim, hidden_dim, bias=False,args=args))
            if (args.enable_mask == True):
                self.linears.append(SparseLinearMulti_mask(hidden_dim, output_dim, bias=False,args=args))
            else:

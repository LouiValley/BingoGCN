...
class FAdmGCN(nn.Module):
    def __init__(self, args):
        ...
        if args.enable_mask == True:
            self.lin = SparseLinearMulti_mask(
                self.num_feats, self.dim_hidden, args=args
            )
        else:
        ...
        for _ in range(self.num_layers - 2):
            if args.enable_mask == True:
                self.lin = SparseLinearMulti_mask(
                    self.dim_hidden, self.dim_hidden, args=args
                )
            else:
                self.lin = SparseLinear(
                    self.dim_hidden, self.dim_hidden, args=args
                )
            self.layers_GCN.append(self.lin)
        ...
        if args.enable_mask == True:
            self.lin = SparseLinearMulti_mask(
                self.dim_hidden, self.num_classes, args=args
            )
        else:
            self.lin = SparseLinear(
                self.dim_hidden, self.num_classes, args=args
    ...
    def forward(self, x, adj, sparsity=None, epoch=None):
        ...
        # x = F.dropout(x, p=self.dropout, training=self.training)
        x = torch.sparse.mm(adj, x)  #  out = AX
        x = self.layers_GCN[-1](x, threshold)  # out = self.lin(x): (AX) W
        return x

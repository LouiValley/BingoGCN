...
def parallel(exp_name, cfg):
    ...
    jobman = JobManager(
        sync_dir, job_id, sync_script_path="utils/sync_jobs.py"
    )
    jobman.clear()
    jobman.start()
    ...
    while True:
        ...
        if no_job_counter >= max_no_job:
            if parallel_counter >= max_parallel:
                print(
                    "[Parallel] Stop searching because searched for enough time"
                )
                break
            else:
        ...
        while True:
            ...
            if resume_job_name is not None:
                ...
                try:
                    ...
                    _update_job(
                        sync_dir, resume_job_name, "completed", job_id, dic
                    )
        ...
        if _check_job_running(jobman, sync_dir, job_name):
            ...
            _update_job(
                sync_dir, job_name, "not completed", job_id, sampled_dic
            )

...
def test(exp_name, cfg, prefix="", epoch=None, use_best=False):
    ...
    outman.print(
        "Number of available gpus: ", torch.cuda.device_count(), prefix=prefix
    )
    ...
    if use_best:
        ...
        dump_path = outman.get_abspath(
            prefix=f"epoch{epoch}.{prefix}", ext="pth"
        )
    else:
    ...
    if os.path.exists(dump_path):
        try:
            ...
            if isinstance(learner.model, DataParallel):
                learner.model.module.load_state_dict(
                    dump_dict["model_state_dict"]
                )
            else:
                learner.model.load_state_dict(dump_dict["model_state_dict"])
        except Exception as e:
            print(
                "[train.py] catched unexpected error in loading checkpoint:",
                str(e),
            )
    ...
    if use_best:
        outman.print(
            "Test Accuracy (Best):", str(result["accuracy"]), prefix=prefix
        )
    else:
    ...
    if use_best:
        output_path = outman.get_abspath(
            prefix=f"test_best.{prefix}", ext="json"
        )
    else:
        output_path = outman.get_abspath(
            prefix=f"test_epoch{epoch}.{prefix}", ext="json"
        )
